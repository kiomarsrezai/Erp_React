...
import ProposalBudgetFormPisnahadi from "./../../components/sections/budget/proposal/proposal-bugetPishnahadi";
...
function BudgetPish() {
  ...
  const getDetailMutation = useMutation(proposalBudgetApi.getDetailDataPish);
...
//   });
  ...
  const queryClient = useQueryClient();
  const getDataMutation = useMutation(transferApi.getModalDataPish, {
    onSuccess: (data) => {
  ...
  const insertCodeAccMutation = useMutation(transferApi.insertCodeAccPish, {
    onSuccess: () => {
      getDataMutation.mutate(formData);
  ...
  const handleOpenDetailModal = (
    ...
    getDetailMutation.mutate({
      ...
      [proposalConfig.description]: row.description,
      [proposalConfig.budgetNext]: row.budgetNext,
    //   description:baseRowData.description,
  ...
  const getInfoDataMutation = useMutation(proposalBudgetApi.getInfoDataPish);
  ...
  const actionButtons = (
    ...
    <Box display={"flex"} justifyContent={"center"}>
      <IconButton
        ...
        size="small"
        onClick={() =>
          insertCodeAccMutation.mutate({
            ...formData,
            codingId: row.codingId
          })
        }
      >
        <AddIcon />
  ...
  return (
    <>
      ...
      <WindowLoading active={  getDataMutation.isLoading ||
          insertCodeAccMutation.isLoading ||getDataMutation.isLoading} />
    </>
  );
}

export default BudgetPish;
