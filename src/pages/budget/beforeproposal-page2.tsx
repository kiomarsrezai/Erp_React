...
import BeforeProposalBudgetForm from "components/sections/budget/beforeproposal2/beforeproposal-budget-form";
...
import BeforeproposalBudgetEdit from "../../components/sections/budget/beforeproposal2/beforeproposal-budget-edit";
import {joinPermissions} from "../../helper/auth-utils";
import {accessNamesConfig} from "../../config/access-names-config";
import SectionGuard from "components/auth/section-guard";
import BeforeproposalBudgetTableRead
  from "../../components/sections/budget/beforeproposal2/beforeproposal-budget-table-read";
import BeforeproposalBudgetChart from "../../components/sections/budget/beforeproposal2/beforeproposal-budget-chart";
...
function BudgetBeforeProposalPage2() {
...
const handleClickDelete = (
  row: TableDataItemShape & GetSingleBeforeProposalItemShape2
) => {
...
const [isOpenModal, setIsOpenModal] = useState(false);
  ...
  const [activeRowData, setActiveRowData] =
    useState<GetSingleBeforeProposalItemShape2 | null>(null);
  ...
  const getDataMutation = useMutation(beforeproposalapi.getData2, {
    onSuccess: (data) => {
  ...
  function showChartModal(row: GetSingleBeforeProposalItemShape2){
  ...
  // data
  const actionButtons = (
    row: (TableDataItemShape & GetSingleBeforeProposalItemShape2) | any
  ) => (
  ...
  const handleClickEditBtn = (row: GetSingleBeforeProposalItemShape2) => {
    if(row.areaId){
  ...
  const handleClickOpenTableReadModal = (row: GetSingleBeforeProposalItemShape2) => {
  ...
  const handleOpenAddCodeModal = (item: GetSingleBeforeProposalItemShape2) => {
    clearTimeout(activeTimeOut.current);
  ...
  const formatTableData = (
    unFormatData: GetSingleBeforeProposalItemShape2[]
  ): TableDataItemShape[] => {
  ...
  const footerMosavabSum = sumFieldsInSingleItemData(
    ...
    "mosavab",
    (item: GetSingleBeforeProposalItemShape2) => item.levelNumber === 1
  );
  ...
  const editButtone = (row: (TableDataItemShape & GetSingleBeforeProposalItemShape2) | any) => {
    return(
        <SectionGuard
            permission={joinPermissions([
              accessNamesConfig.BUDGET__BeforePROPOSAL_PAGE,
    ...
    <>
      ...
      >
        <ProposalModal1
          ...
          formData={formData}
          baseRowData={activeRowData as GetSingleBeforeProposalItemShape2}
      ...
      >
        <ProposalModalInsertCode
          activeRowData={activeRowData as GetSingleBeforeProposalItemShape2}
          formData={formData}
...
export default BudgetBeforeProposalPage2;
